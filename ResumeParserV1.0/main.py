...
def add_to_db(data, skills, linkedin, file_path):
    ...
    try:
        ...
        sql = "INSERT INTO resume(`name`, `email`, `phone`, `linkedin`, `university`, `degree`, `designition`, `companies`, `exp`, `file_path`, `date_added`, `checked`)" \
        ...
        val = (data['name'], data['email'], data['phone'], linkedin, str(data['university']),
               ...
               data['total_exp'], file_path, now.strftime("%d/%m/%Y %H:%M:%S"), False)
...
def detect_photo(file_path, id):
    ...
    for page_index in range(len(pdf_file)):
        ...
        for image_index, img in enumerate(image_list, start=1):
            ...
            # save it
            image.save(open(f"{img_tmp}\image_{id}_{page_index + 1}_{image_index}.{image_ext}", "wb"))
            image.close()
...
def extractData(file_path):
    ...
    #
    all_skills = extract_skills(translation)
    skills = all_skills['full_matches']
    skills.extend(all_skills['ngram_scored'])
    return resumeparse.read_file('translation.txt'), skills, extract_linkedin(translation)
...
def Re_extract(resume_id, file_name):
    ...
    try:
        ...
        unique = list({each['skill_id']: each for each in skills}.values())
        num = add_to_db(data, unique, linkedin, os.path.join(directory, file_name))
        os.rename(os.path.join(img_directory, str(resume_id)) + ".jpg", os.path.join(img_directory, str(num)) + ".jpg")
...
def extract():
    try:
        for filename in os.listdir(app.config["UPLOAD_FOLDER"]):
            ...
            unique = list({each['skill_id']: each for each in skills}.values())
            resume_id = add_to_db(data, unique, linkedin, os.path.join(directory, filename))
            save_photo(os.path.join(directory, filename), resume_id)
...
def DeleteResume(resume_id):
    ...
    if not record[0]:
        ...
        os.remove(os.path.join(img_directory, resume_id) + ".jpg")
        return jsonify({
            'status': 'success',
            'message': 'deleted'
        })
